...
export async function POST(req: NextRequest) {
  try {
    ...
    if (body.snapshot) {
      ...
      `, s.year_month, s.cash_balance, s.bank_balance, s.accounts_receivable, s.total_revenue, s.total_expense, s.net_income);
    }
    ...
    if (body.service_pl && Array.isArray(body.service_pl)) {
      for (const pl of body.service_pl) {
        ...
        `, pl.year_month, pl.service_name, pl.revenue, pl.cost, pl.gross_profit);
      }
    }
    ...
    if (body.expenses && Array.isArray(body.expenses)) {
      for (const e of body.expenses) {
        ...
        `, e.year_month, e.category, e.sub_category || "", e.amount, e.is_fixed ? 1 : 0);
      }
    }
    ...
    if (body.project_pl && Array.isArray(body.project_pl)) {
      for (const p of body.project_pl) {
        ...
        `, p.year_month, p.municipality, p.service_name, p.revenue || 0, p.cost || 0, grossProfit, p.notes || null);
      }
    }
    ...
    if (body.funding_items && Array.isArray(body.funding_items)) {
      for (const f of body.funding_items) {
        ...
        `, f.year_month, f.section, f.category, f.sub_category || "", f.amount, f.is_actual ? 1 : 0, f.planned_amount ?? null);
      }
    }
    ...
    if (body.funding_balances && Array.isArray(body.funding_balances)) {
      for (const b of body.funding_balances) {
        ...
        `, b.year_month, b.opening_balance);
      }
    }
      ...
      for (const r of body.receivables) {
        await dbRun(`
          ...
          ON CONFLICT(due_date, source) DO UPDATE SET
            ...
            amount = excluded.amount,
            notes = excluded.notes
        `, r.due_date, r.source, r.service_name || null, r.amount, r.status || "pending", r.notes || null);
      }
    }
    ...
    if (body.bs) {
      ...
      await dbRun(`
        ...
        ON CONFLICT(year_month) DO UPDATE SET
          ...
          synced_at = datetime('now', 'localtime')
      `, b.year_month, b.total_assets, b.cash, b.receivables_total, b.payables, b.loan_balance, b.net_assets);
    }

...
export async function GET() {
  const db = getDb();
  const rows = db.prepare("SELECT * FROM project_profitability ORDER BY year_month DESC, municipality ASC").all();
...
export async function POST(req: NextRequest) {
  try {
    ...
    `).run(year_month, municipality, service_name, revenue || 0, cost || 0, gross_profit, notes || null);
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    const db = getDb();
    db.prepare("DELETE FROM project_profitability WHERE id = ?").run(id);
    return NextResponse.json({ ok: true });
  } catch (err: unknown) {
    const message = err instanceof Error ? err.message : "Unknown error";
    return NextResponse.json({ ok: false, error: message }, { status: 500 });
